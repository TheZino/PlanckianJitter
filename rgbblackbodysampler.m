function BB_sRGB = rgbblackbodysampler(wavelengths, temperatures)
%RGBBLACKBODYSAMPLER Sample the sRGB equivalent of a Black Body radiator
%
%   BB_sRGB = RGBBLACKBODYSAMPLER(wavelengths, temperatures) samples the
%   sRGB equivalent of a Black Body radiator with Correlated Color
%   Temperature (CCT) specified by temperatures, and discretizing the
%   spectrum according to wavelengths.
%
%   wavelenghts must be a vector, expressed in nanometers. Default value is
%   400:10:700.
%   temperatures must be a vector, expressed in Kelvin. Default value is
%   3000:500:15000.

    % Verify input data
    if nargin < 2
        temperatures = 3000:500:15000;
    end
    if nargin < 1
        wavelengths = 400:10:700;
    end
    
    % Sample a new illuminant spectrum from the distribution of a black body radiator
    BB_spectrum = blackbody(wavelengths, temperatures);

    % Transform the sampled spectrum into its sRGB representation
        % Map the spectrum into the corresponding XYZ stimuli and normalize this tristimulus by its Y component
        [lambda, xFcn, yFcn, zFcn] = colorMatchFcn('CIE_1931');
        XYZ = interp1(lambda, [xFcn; yFcn; zFcn]', wavelengths);
        BB_XYZ = BB_spectrum'*XYZ./(BB_spectrum'*XYZ(:,2));
        % Convert into CIE 1976 L*a*b color space, and fix its L component to 50 in a 0-to-100 scale. 
        BB_LAB = xyz2lab(BB_XYZ);
        BB_LAB(:,1) = 50;
        % Convert the resulting values in sRGB
        BB_sRGB = lab2rgb(BB_LAB);

end

function spectralBB = blackbody(wavelengths, temperatures)

    % Force wavelength to column vector, and temperatures to row vector
    wavelengths = wavelengths(:) * 1e-9;
    temperatures = temperatures(:)';

    % Planck's Law constants (source: brucelindbloom.com)
    c = 2.99792458e8;
    h = 6.626176e-34;
    k = 1.380662e-23;
    c1 = 2*pi*h*c^2;
    ...
    % Planck's Law
    spectralBB = (c1 ./ ( repmat(wavelengths.^5, [1 length(temperatures)]) .* (exp(c2./(wavelengths*temperatures))-1) )) * 1e-9 / pi;
...
% Jeff Mather (2022). Spectral and XYZ Color Functions (https://www.mathworks.com/matlabcentral/fileexchange/7021-spectral-and-xyz-color-functions), MATLAB Central File Exchange. Retrieved March 3, 2022.
